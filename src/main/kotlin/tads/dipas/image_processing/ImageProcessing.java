...
public class ImageProcessing {
        ...
        try {
            ...
            for (int i = 0; i < im.getHeight() - 1; i++) {
                for (int j = 0; j < im.getWidth() - 1; j++) {
                    //int rgb = im.getRGB((im.getHeight()-1)-i, (im.getWidth()-1)-j);
    ...
    public static BufferedImage im2BufferedImage(int[][][] im) {
    ...
    public static BufferedImage im2BufferedImage(boolean[][] im) {
        BufferedImage buff = new BufferedImage(im[0].length, im.length, BufferedImage.TYPE_INT_RGB);
        int val;
        for (int i = 0; i < im.length; i++) {
            for (int j = 0; j < im[0].length; j++) {
    ...
    public static void imWrite(int[][][] im, String path) {
        imWrite(path, im2BufferedImage(im));
    }
    ...
    public static void imShow(int[][][] im, String name) {
        imShow(name, im2BufferedImage(im));
    }
    ...
    public static boolean[][] bwErode(boolean[][] bw, int tam) {
        ...
        erosion = ImageProcessing.copy(bw);
        boolean flag;
    ...
    public static boolean[][] bwDilate(boolean[][] bw, int tam) {
        ...
        dilatation = ImageProcessing.copy(bw);
    ...
    public static boolean[][] bwOpen(boolean[][] im, int n) {
        return ImageProcessing.bwDilate(ImageProcessing.bwErode(im, n), n);
    }

    public static boolean[][] bwClose(boolean[][] im, int n) {
        return ImageProcessing.bwErode(ImageProcessing.bwDilate(im, n), n);
    }

    public static boolean[][] bwLine(boolean[][] im, int[] dots) {
        im = ImageProcessing.copy(im);
        ...
        do {
            ...
            for (int i = 1; i < output.length - 1; i++) {
                for (int j = 10; j < output[0].length - 1; j++) {
                    if (output[i][j]) {
                        boolean[][] neighbours = ImageProcessing.neighbours(output, i, j);
    ...
    public static ArrayList<Properties> regionProps(boolean[][] im) {
        int[][] lb = ImageProcessing.bwLabel(im);
        ArrayList<Properties> prop = new ArrayList<>();

        int qtd = ImageProcessing.max(lb);
        ...
        for (int r = 1; r <= qtd; r++) {
            ...
            p.image = ImageProcessing.imCrop(imRegion, minRow, minCol, maxRow, maxCol);
    ...
    public static int[][] sub(int[][] im1, int[][] im2) {
        int[][] result = ImageProcessing.copy(im1);
        for (int i = 0; i < im1.length; i++)
            for (int j = 0; j < im1[0].length; j++)
                result[i][j] = im1[i][j] - im2[i][j];
        return result;
    ...
    public static boolean[][] sub(boolean[][] im1, boolean[][] im2) {
        boolean[][] result = ImageProcessing.copy(im1);
        for (int i = 0; i < im1.length; i++)
            for (int j = 0; j < im1[0].length; j++)
    ...
    public static int[][][] _imResize(int[][][] im) {// especificamente para o SAPL
